...
public:
	...
	void display() override {
		...
		if (ImGui::Begin(title.c_str(), &stay_open)) {
			...
			ui_signal(220, "RS0", SIGNAL_NEXT_BOOL(rs0), ACTHI_ASSERT);
			ui_signal(220, "RS1", SIGNAL_NEXT_BOOL(rs1), ACTHI_ASSERT);
			ui_signal(220, "RS2", SIGNAL_NEXT_BOOL(rs2), ACTHI_ASSERT);
			ui_signal(220, "RS3", SIGNAL_NEXT_BOOL(rs3), ACTHI_ASSERT);

			// right column
			ImGui::SetCursorPos(origin);
			ui_signal(330, "CA1", SIGNAL_NEXT_BOOL(ca1), ACTHI_ASSERT);
			ui_signal(330, "CA2", SIGNAL_NEXT_BOOL(ca2), ACTHI_ASSERT);
			ui_signal(330, "CB1", SIGNAL_NEXT_BOOL(ca1), ACTHI_ASSERT);
			ui_signal(330, "CB2", SIGNAL_NEXT_BOOL(ca2), ACTHI_ASSERT);
			ui_signal(330, "/IRQ", SIGNAL_NEXT_BOOL(irq_b), ACTLO_ASSERT);
