...
MunitResult test_count(const MunitParameter params[], void* user_data_or_fixture) {
	...
	munit_assert_size(filt_6502_asm_count_instruction(TEST_BINARY, sizeof(TEST_BINARY), 0, sizeof(TEST_BINARY)), ==, 13);
	munit_assert_size(filt_6502_asm_count_instruction(TEST_BINARY, sizeof(TEST_BINARY), 8, 20), ==, 4);
